# CookPadの最近見たレシピを取得する。
# 自分のアカウントでログインして、最近みたレシピの名前とURLを取得し、最大20件表示される。

# 正しいページに繊維していることを確認するために、assert文でページのURLやタイトルやタイトルを確認している。
# (MechanicalSoupは視覚的なフィードバックがないため、注意が必要)

# 実行方法
# forego run python cookpad_recent_recipes.py
