...
# wikiExtractor.pyを実行するとwikipediaのダンプファイルをテキストに変換できる。

# 実行方法　例
# オプション
# --no_template：ページの冒頭などに貼られるテンプレートを展開しないことを指示する
# -o：出力先のディレクトリを指定する
# -b：分割するファイルのサイズ(この例では100MB)ｗ指定する
# python WikiExtractor.py --no_template -o article -b 100M 〇〇pages-articles.xml-〇〇.bz2

# 注意点
# 記事中の見出しは取り除かれる：--sectionsオプションで含めることが可能
# 表や箇条書きは取り除かれる：--listsオプションで箇条書きを含めることが可能
# MediaWikiのテンプレートが使われている箇所は取り除かれる：--no_templatesオプションを外すとテンプレートが展開されるようになるが、所要時間が増えるので注意が必要
