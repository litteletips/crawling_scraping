# 不特定多数のページをクロールSpider

# start_urls属性に新着エントリーページを指定し、parse()でそのレスポンスをパースする
# parse_page()では、個別のWebページを処理する。

# ここでWebサイト特有の処理を何も行っていないことがポイント
...
class BroadSpider(scrapy.Spider):
    ...
    # 次のページに行くと、次のようなURLになる。http://b.hatena.ne.jp/entrylist/all?page=2
