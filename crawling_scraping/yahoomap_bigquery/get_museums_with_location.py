# 日本の美術館の位置情報を取得する。
# SPARQLで美術館の情報を取得し、位置情報を持たない美術館については住所をジオコーディングして位置情報を取得するスクリプト。

# 取得した位置情報は、GeoJSON形式でmuseums.geojsonという名前のファイルに書き出す。
# (GeoJSONは様々な地理的な情報を格納するためのフォーマットで、一定のルールに従ったJSON)
