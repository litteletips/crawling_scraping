# MeCabを用いて、Wikipediaの文章から頻出単語上位30位を表示する。
# iter_doc_contents()：for文をつかって①行ずつ読み込みをする。処理に必要な部分だけをメモリに読み込むことで、メモリの消費を抑える。結果的にスワップアウトプットが減り、高速に処理できる。
# get_words()：node.featureから品詞の情報を取り出し、固有名詞または一般名詞の場合だけ処理するようにしている。「は」「を」のような助詞や句読点が頻出単語として処理するようにすることを防ぐ。
